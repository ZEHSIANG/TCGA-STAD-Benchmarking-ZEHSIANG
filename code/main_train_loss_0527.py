...
MASTER_LOG_FILE = '/mnt/data2/TCGA_STAD_Project/0527_experiments/experiment_result/loss_comparison_result_0527.csv'
FAILED_LOG_FILE = '/mnt/data2/TCGA_STAD_Project/0527_experiments/experiment_result/failed_experiments_log.csv'

def train_epoch(model, loader, optimizer, criterion, device, accum_steps, mode, loss_fn_name="cox"):
    model.train()
    total_loss = 0
    ...
    for i, batch_data in enumerate(loader):
        ...
        if loss_fn_name in ['deephit', 'mtlr', 'logistic_hazard']:
            with torch.no_grad():
                risk = criterion.get_risk_score(logits.detach())
            all_scores.extend(risk.cpu().numpy().flatten())
        else:
...
def validate(model, loader, device, mode, criterion, loss_fn_name="cox"):
    model.eval()
    total_loss = 0
    ...
    with torch.no_grad():
        for batch_data in loader:
            ...
            # DeepHit/MTLR 輸出10維，取累積概率作為風險分數
            if loss_fn_name in ['deephit', 'mtlr', 'logistic_hazard']:
                risk = criterion.get_risk_score(logits)
                all_scores.extend(risk.cpu().numpy().flatten())
            else:
...
def main():
    ...
    parser.add_argument('--loss_function', type=str, default='cox', choices=['cox', 'ranking', 'deephit', 'mtlr', 'logistic_hazard'])
    ...
    try:
        ...
        train_set = TCGADataset(DEFAULT_CSV_DIR, current_feat_dir, args.fold, logic_mode, 'train', args.max_patches, 
        ...
        val_set = TCGADataset(DEFAULT_CSV_DIR, current_feat_dir, args.fold, logic_mode, 'val', args.max_patches, 
        ...
        # 🟢 [修正 3] 最後防呆檢查
        # DeepHit / MTLR 需要 10 維輸出，動態替換最後一層
        NUM_BINS = 10
        if args.loss_function in ["deephit", "mtlr", "logistic_hazard"]:
            def replace_last_linear(module, num_bins):
                for name, child in reversed(list(module.named_children())):
                    if isinstance(child, torch.nn.Linear):
                        setattr(module, name, torch.nn.Linear(child.in_features, num_bins))
                        print(f"  替換輸出層: {name} → Linear({child.in_features}, {num_bins})")
                        return True
                    if replace_last_linear(child, num_bins):
                        return True
                return False
            replace_last_linear(model, NUM_BINS)
            model = model.to(device)
            print(f"輸出層已替換為 {NUM_BINS} 維")

        if model is None:
            raise RuntimeError(f"Model initialization failed! logic_mode={logic_mode}, input_mode={args.input_mode}")

        optimizer = torch.optim.Adam(model.parameters(), lr=args.lr)    
        if args.loss_function == 'cox':
            criterion = myLoss_0527.CoxPHLoss()
        elif args.loss_function == 'ranking':
            criterion = myLoss_0527.RankingLoss(margin=1.0)
        elif args.loss_function == 'deephit':
            criterion = myLoss_0527.DeepHitLoss(num_bins=10, alpha=0.5, sigma=0.1)
        elif args.loss_function == 'logistic_hazard':
            criterion = myLoss_0527.LogisticHazardLoss(num_bins=NUM_BINS)
        elif args.loss_function == 'mtlr':
            criterion = myLoss_0527.MTLRLoss(num_bins=10)
        ...
        for epoch in range(args.epochs):
            train_loss, train_c_index = train_epoch(model, train_loader, optimizer, criterion, device, accum_steps, logic_mode, args.loss_function)
            val_loss, val_c_index, val_ids, val_scores, val_events = validate(model, val_loader, device, logic_mode, criterion, args.loss_function)
        ...
        with open(MASTER_LOG_FILE, 'a', newline='') as f:
            writer = csv.writer(f)
            if not file_exists:
                writer.writerow(['Timestamp', 'Input Mode', 'Model', 'Encoder', 'Fold', 'BatchSize', 'MaxPatches', 'Loss Function', 'Best Val C-Index', 'Best Epoch', 'Log Path'])
            writer.writerow([datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), args.input_mode, args.model_type, args.encoder_name, args.fold, bs_name, args.max_patches, args.loss_function, f"{best_c_index:.4f}", best_epoch, save_path])
